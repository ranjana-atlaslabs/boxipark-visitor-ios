...
struct ResturentAPI {
    ...
    static func accountInformation(completion: @escaping (AccountInformationResult?, Error?, Int?) -> Void) {
        ...
        let url = Constant.PAYTRONIX_BASE_URL + "guest/accountInformation.json?merchantId=\(Constant.MERCHANT_ID)&access_token=\(accessToken)&username=\(username)&authentication=\(authenticationType)&printedCardNumber=\(printedCardNumber)"
